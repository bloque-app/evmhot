...
async fn test_db_operations() {
    ...
    let fetched_addr = db.get_account_by_id(id).await.unwrap().unwrap().1;
    assert_eq!(fetched_addr, address);

    let fetched_id = db.get_account_by_address(address).await.unwrap().unwrap();
    assert_eq!(fetched_id, id);

    // Test Index Increment
    let next_idx = db.get_next_derivation_index().await.unwrap();
    assert_eq!(next_idx, 1);

    // Test Deposits
    let tx_hash = "0xabc";
    let amount = "100";
    db.record_deposit(tx_hash, id, amount).await.unwrap();
    ...
    // Test Sweep Mark
    db.mark_deposit_swept(tx_hash).await.unwrap();
    let deposits_after = db.get_detected_deposits().await.unwrap();
...
async fn test_monitor_creation_with_http_provider() {
    ...
    let db = Db::new(db_file.path().to_str().unwrap()).await.unwrap();
...
#[tokio::test]
async fn test_monitor_db_operations() {
    ...
    let deposits_before = db.get_detected_deposits().await.unwrap();
    ...
    // Test block tracking
    db.set_last_processed_block(100).await.unwrap();
    assert_eq!(db.get_last_processed_block().await.unwrap(), 100);
}

#[tokio::test]
async fn test_monitor_address_lookup() {
    ...
    let account = db.get_account_by_address(&addr1).await.unwrap();
    assert!(account.is_some());
    assert_eq!(account.unwrap(), "user1");

    // Check addr2 is not registered
    let account2 = db.get_account_by_address(&addr2).await.unwrap();
...
#[tokio::test]
async fn test_sweeper_deposit_workflow() {
    ...
    db.mark_deposit_swept("0xtx123").await.unwrap();
    let deposits_after = db.get_detected_deposits().await.unwrap();
    assert_eq!(deposits_after.len(), 0);

    // Verify the account details are correct for deriving keys
    let account_details = db.get_account_by_id("test_user").await.unwrap().unwrap();
...
#[tokio::test]
async fn test_sweeper_multiple_deposits() {
    ...
    db.mark_deposit_swept("0xtx1").await.unwrap();
    let deposits_after_1 = db.get_detected_deposits().await.unwrap();
    assert_eq!(deposits_after_1.len(), 2);

    db.mark_deposit_swept("0xtx2").await.unwrap();
    let deposits_after_2 = db.get_detected_deposits().await.unwrap();
    assert_eq!(deposits_after_2.len(), 1);

    db.mark_deposit_swept("0xtx3").await.unwrap();
    let deposits_after_3 = db.get_detected_deposits().await.unwrap();
...
#[tokio::test]
async fn test_erc20_deposit_failure_tracking() {
    let db_file = NamedTempFile::new().unwrap();
    let db = Db::new(db_file.path().to_str().unwrap()).await.unwrap();
    ...
    assert_eq!(db.get_detected_erc20_deposits().await.unwrap().len(), 1);

    for i in 1..=5 {
        let count = db.increment_sweep_failure_count("0xabc:1").await.unwrap();
        assert_eq!(count, i);
    }

    db.mark_erc20_deposit_failed("0xabc:1").await.unwrap();

    assert_eq!(db.get_detected_erc20_deposits().await.unwrap().len(), 0);
}

#[tokio::test]
async fn test_erc20_bulk_mark_failed_for_account_token() {
    let db_file = NamedTempFile::new().unwrap();
    let db = Db::new(db_file.path().to_str().unwrap()).await.unwrap();
    ...
    assert_eq!(db.get_detected_erc20_deposits().await.unwrap().len(), 3);
    ...
    let remaining = db.get_detected_erc20_deposits().await.unwrap();
